...
--create-namespace --namespace kafka --version 0.35.1 -f strimzi-config.yaml
...
# Wait for opentelemetry-operator webhook to be ready before creating OpenTelemetryCollector resources.
# Without this wait, the mutating webhook (mopentelemetrycollectorbeta.kb.io) with failurePolicy: Fail
# will reject the collector creation because the operator pod's webhook endpoint is not yet available.
...
kubectl wait --for=jsonpath='{.subsets[0].addresses[0].ip}' \
  endpoints/opentelemetry-operator-webhook -n observability --timeout=120s
...
# Install istiod control plane with ambient mode
helm upgrade --install istiod istio/istiod -n istio-system \
  --set profile=ambient \
  --set global.platform=k3s \
  --wait

# Install istio-cni node agent
...
# Apply PeerAuthentication PERMISSIVE so Traefik (outside the mesh) can still reach services
kubectl apply -f - <<'EOF'
...
# Enroll namespaces into the ambient mesh
# Kafka is intentionally excluded because Strimzi manages its own TLS
for ns in yas postgres redis elasticsearch keycloak observability; do
  kubectl label ns --overwrite "$ns" istio.io/dataplane-mode=ambient
done

# Install Kiali (observability dashboard for Istio)
helm repo add kiali https://kiali.org/helm-charts
helm repo update

# Install operator only (no CR)
helm upgrade --install kiali-operator kiali/kiali-operator \
  --namespace kiali-operator --create-namespace --wait

# Apply Kiali CR with configuration (includes ingress, web_fqdn, prometheus, grafana)
kubectl apply -f ./kiali/cr.yaml
kubectl rollout status deployment -n istio-system kiali --timeout=120s

# Configure Prometheus to scrape Istio metrics
kubectl apply -f ./observability/prometheus/istio-monitoring.yaml
